...
docker build --progress=plain -f Dockerfile -t technoboggle/haproxy-alpine:2.7-3.15.5 --build-arg buildDate=$(date +'%Y-%m-%d') --no-cache --progress=plain . 
docker run -it -d -p 8010:80 -p 4430:443 --rm --name haproxy technoboggle/haproxy-alpine:2.7-3.15.5
docker tag technoboggle/haproxy-alpine:2.7-3.15.5 technoboggle/haproxy-alpine:latest
docker login
docker push technoboggle/haproxy-alpine:2.7-3.15.5
